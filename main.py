...
class AgentRegistrationRequest(BaseModel):
    ...
    config: Optional[Dict[str, Any]] = Field(None, description="Configuración del agente")
...
async def load_agents_from_registry():
    ...
    if not registry_path.exists():
        logger.warning(f"Registry file {registry_file} not found, creating default registry")
    ...
    for entry in agent_registry:
        try:
            ...
            logger.info(f"Agent {agent_id} ({agent_class_name}) registered successfully")
