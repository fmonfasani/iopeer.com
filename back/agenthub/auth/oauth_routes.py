...
async def google_login():
    ...
    try:
        ...
        return RedirectResponse(
            url=f"{FRONTEND_URL}/login?error=google_error", status_code=302
        )
...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if error:
            ...
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=google_denied", status_code=302
            )

        if not code:
            logger.error("No authorization code from Google")
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=no_google_code", status_code=302
            )
        ...
        if not token_data:
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=google_token_failed", status_code=302
            )
        ...
        if not user_info:
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=google_user_failed", status_code=302
            )
        ...
        return RedirectResponse(
            url=f"{FRONTEND_URL}/login?error=google_callback_error", status_code=302
        )
...
async def github_login():
    ...
    if not config["github_configured"]:
        ...
        return RedirectResponse(
            url=f"{FRONTEND_URL}/login?error=github_not_configured", status_code=302
        )
    ...
    try:
        ...
        return RedirectResponse(
            url=f"{FRONTEND_URL}/login?error=github_error", status_code=302
        )
...
async def github_callback(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if error:
            ...
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=github_denied", status_code=302
            )

        if not code:
            logger.error("No authorization code from GitHub")
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=no_github_code", status_code=302
            )
        ...
        if not token_data:
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=github_token_failed", status_code=302
            )
        ...
        if not user_info:
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=github_user_failed", status_code=302
            )
        ...
        if not user_info.get("email"):
            return RedirectResponse(
                url=f"{FRONTEND_URL}/login?error=no_github_email", status_code=302
            )
        ...
        return RedirectResponse(
            url=f"{FRONTEND_URL}/login?error=github_callback_error", status_code=302
        )
