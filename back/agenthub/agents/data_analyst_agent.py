...
class DataAnalystAgent(BaseAgent):
            ...
            "data": {
                ...
                "summary": {
                    ...
                    "improving_metrics": len(
                        [m for m in analysis_results.values() if m["trend"] == "up"]
                    ),
                    "declining_metrics": len(
                        [m for m in analysis_results.values() if m["trend"] == "down"]
                    ),
    ...
    def _create_report(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sections = [
            ...
            },
            {
                ...
                "charts": (
                    self._generate_metrics_charts(metrics) if include_charts else []
                ),
                "order": 2,
            },
            {
            ...
            "data": {
                ...
                "summary": {
                    ...
                    "above_target": len(
                        [
                            k
                            for k in calculated_kpis.values()
                            if k["status"] == "above_target"
                        ]
                    ),
                    "on_target": len(
                        [
    ...
    def _forecast_trends(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        trend = (
            (historical_data[-1] - historical_data[0]) / len(historical_data)
            ...
            "data": {
                ...
                "summary": {
                    ...
                    "improved_metrics": len(
                        [m for m in comparison_results.values() if m["trend"] == "up"]
                    ),
                    "declined_metrics": len(
                        [m for m in comparison_results.values() if m["trend"] == "down"]
                    ),
                    "stable_metrics": len(
                        [
                            m
        ...
        return {
            ...
            "data": {
                ...
                "summary": {
                    ...
                    "high_confidence": len(
                        [i for i in insights if i["confidence"] > 0.8]
                    ),
    ...
    def _generate_metric_insights(self, analysis_results: Dict) -> List[str]:
        ...
        improving_metrics = [
            name for name, data in analysis_results.items() if data["trend"] == "up"
        ]
        declining_metrics = [
            name for name, data in analysis_results.items() if data["trend"] == "down"
        ]

        if improving_metrics:
            insights.append(
                f"📈 Métricas en crecimiento: {', '.join(improving_metrics[:3])}"
            )

        if declining_metrics:
            insights.append(
                f"📉 Métricas que requieren atención: {', '.join(declining_metrics[:3])}"
            )
        ...
        if volatile_metrics:
            insights.append(
                f"⚠️ Métricas con alta volatilidad: {', '.join(volatile_metrics)}"
            )
    ...
    def _generate_recommendations(self, analysis_results: Dict) -> List[str]:
        ...
        if critical_metrics:
            recommendations.append(
                f"🚨 Atención urgente requerida para: {', '.join(critical_metrics)}"
            )
        ...
        if excellent_metrics:
            recommendations.append(
                f"✅ Mantener estrategias exitosas en: {', '.join(excellent_metrics)}"
            )
    ...
    def _calculate_overall_performance(self, analysis_results: Dict) -> str:
        ...
        total_score = sum(
            status_scores.get(data["status"], 3) for data in analysis_results.values()
        )
    ...
    def _create_dashboard_widgets(
        self, dashboard_type: str, metrics: List[str]
    ) -> List[Dict]:
    ...
    def _generate_widget_data(self, widget: Dict) -> Dict:
        ...
        if widget_type == "kpi":
            ...
            return {
                "data": [
                    {"category": f"Cat {i}", "value": random.randint(50, 500)}
                    for i in range(6)
                ]
            }
        else:
    ...
    def _calculate_global_kpi_score(self, calculated_kpis: Dict) -> Dict:
        ...
        total_score = sum(
            status_scores.get(kpi["status"], 60) for kpi in calculated_kpis.values()
        )
        ...
        return {
            ...
            "status": (
                "healthy"
                if avg_score >= 80
                else "needs_attention" if avg_score >= 60 else "critical"
            ),
        }
    ...
    def _generate_kpi_recommendations(self, calculated_kpis: Dict) -> List[str]:
        ...
        below_target = [
            name
            for name, kpi in calculated_kpis.items()
    ...
    def _generate_segment_insights(self, segment_results: Dict) -> List[str]:
        ...
        highest_conversion = max(
            segment_results.values(), key=lambda x: x["metrics"]["conversion_rate"]
        )
    ...
    def _generate_correlation_insights(
        self, significant_correlations: List[Dict]
    ) -> List[str]:
        """Genera insights de correlación"""
        insights = []

        strong_correlations = [
            c for c in significant_correlations if c["strength"] == "strong"
        ]
        if strong_correlations:
            insights.append(
                f"Se encontraron {len(strong_correlations)} correlaciones fuertes"
            )

        positive_correlations = [
            c for c in significant_correlations if c["direction"] == "positive"
        ]
        insights.append(
            f"{len(positive_correlations)} correlaciones positivas identificadas"
        )
